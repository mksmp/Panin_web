...
function autoCompleteInput(event) { // вызов подгрузки и генерации списка при изменении инпута
    let input = document.querySelector('input');
    let requestUrl = "http://cat-facts-api.std-900.ist.mospolytech.ru/autocomplete?q=" + input.value;
    if (event.target) {
        let ul = document.getElementById('list'); 
        ul.innerHTML = ''; // очистка списка перед тем как будет новый
        downloadDataInput(requestUrl);
    }
}

function downloadDataInput(requestUrl) { // загружает данные по запросам при изменении инпута
    let xhr = new XMLHttpRequest();
...
function renderListInput(arrayInput) { // рендерит выпадающий список
...
function clickList(event) { // поиск по выпадающему списку
    let li = event.target;
    let input = document.querySelector('input');
...
window.onload = function () {
    ...
    searchbtn.addEventListener('click', clickHandler); // поиск по кнопке
    document.getElementById('search-field').oninput = autoCompleteInput; // выпадающий список
    let listelem = document.querySelector('.list-input'); // поиск по выпадающему списку
    listelem.addEventListener('click', clickList); // поиск по выпадающему списку
}
